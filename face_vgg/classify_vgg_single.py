...
"""
Modified on 2018 for academic use
...
def classify_video(frames, net, transformer, is_flow):
  ...
  fc6_features = np.zeros((num_frames, 4096))
  predict_individual =[]
  index = 0
  for im in frames:   
    ...
    out = net.forward()
    fc6_features[index] = net.blobs['fc6'].data[0]
    predict_image[index] = out['probs']
    predict_individual.append(argmax(out['probs'])) 
    sp = re.split('/',im)
...
def get_face_prob():
  ...
  fc6_whole= np.zeros((lcount,4096))
  ...
  for iname in sorted(os.listdir(val_dir)):
    ...
    if os.path.exists(os.path.join(val_face_dir,fname3)): 
        ...
        fc6_whole[num_count,:]=np.zeros((1,4096))
        num_count += 1     
        print iname 

  del RGB_net
  np.save('vgg_face.npy', weighted_pred_faces)
